...
void sighup ()
{
  ...
  vty_serv_sock (vty_addr, vty_port, RSVPD_VTYSH_PATH);
}
...
#include "sigevent.h"
struct quagga_signal_t rsvpd_signals[] = 
{
 { .signal = SIGHUP,
   .handler = &sighup,
 },
...
Daemon which manages rsvpd related configuration.\n\n\
...
int main (int argc, char **argv)
{
  ...
  zlog_default = openzlog (progname, ZLOG_STDOUT, ZLOG_RSVPD,
...
while (1) {
	...
	case 'P':
	  ...
	  break;
case 'u': rsvpd_privs.user = optarg; break;
case 'g': rsvpd_privs.group = optarg; break;
	case 'v':
...
//signal_init();
signal_init (master, Q_SIGC(rsvpd_signals), rsvpd_signals);
  cmd_init(1);
  vty_init(master);
  memory_init();

  /* RSVPD related initialization. */
  rsvpd_init();
  rsvpd_zclient_init();
