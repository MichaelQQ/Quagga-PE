...
void sighup ()
{
  ...
  vty_serv_sock (vty_addr, vty_port, BRCTLD_VTYSH_PATH);
}
...
int main (int argc, char **argv)
{
  ...
  zlog_default = openzlog (progname, ZLOG_STDOUT, ZLOG_BRCTLD,
...
//signal_init();
signal_init (master, Q_SIGC(brctld_signals), brctld_signals);
  cmd_init(1);
  vty_init(master);
  memory_init();
