...
static int br_cmd_addbr(int argc, char*const* argv)
{
	...
	case EEXIST:
		...
		sprintf(brctl_meg_buf,"device %s already exists; can't create\r\nbridge with the same name\r\n", argv[1]);
		return 1;
	default:
		fprintf(stderr, "add bridge failed: %s\n",
			strerror(err));
		sprintf(brctl_meg_buf,"add bridge failed: %s\r\n",strerror(err));
		return 1;
	}
}
...
static int br_cmd_delbr(int argc, char*const* argv)
{
	...
	case ENXIO:
		fprintf(stderr, "bridge %s doesn't exist; can't delete it\n",
			argv[1]);
		sprintf(brctl_meg_buf,"bridge %s doesn't exist; can't delete it\r\n",
		 argv[1]);
		return 1;
	...
	case EBUSY:
		fprintf(stderr, "bridge %s is still up; can't delete it\n",
			argv[1]);
		sprintf(brctl_meg_buf,"bridge %s is still up; can't delete it\r\n",
		 argv[1]);
		return 1;
	...
	default:
		fprintf(stderr, "can't delete bridge %s: %s\n",
			argv[1], strerror(err));
		sprintf(brctl_meg_buf,"can't delete bridge %s: %s\r\n",
		 	argv[1], strerror(err));
		return 1;
	}
}
...
static int br_cmd_addif(int argc, char *const* argv)
{
	...
	while (argc-- > 0) {
		...
		case ENODEV:
			...
			sprintf(brctl_meg_buf,"interface %s does not exist!\r\n", ifname);
			break;
		...
		case EBUSY:
			fprintf(stderr,	"device %s is already a member of a bridge; "
				...
				brname);
			sprintf(brctl_meg_buf,"device %s is already a member of a bridge; "
				"can't enslave it to bridge %s.\r\n", ifname,
				brname);
			break;
		...
		case ELOOP:
			fprintf(stderr, "device %s is a bridge device itself; "
				...
				ifname);
			sprintf(brctl_meg_buf,"device %s is a bridge device itself; "
				"can't enslave a bridge device to a bridge device.\r\n",
				ifname);
			break;
		...
		default:
			...
			sprintf(brctl_meg_buf,"can't add %s to bridge %s: %s\r\n",
				ifname, brname, strerror(err));
		}
		return 1;
	}
	...
	while (argc-- > 0) {
		...
		case EINVAL:
			fprintf(stderr, "device %s is not a slave of %s\n",
				ifname, brname);
			sprintf(brctl_meg_buf,"device %s is not a slave of %s\r\n",
				ifname, brname);
			break;
		...
		default:
			fprintf(stderr, "can't delete %s from %s: %s\n",
				ifname, brname, strerror(err));
			sprintf(brctl_meg_buf,"can't delete %s from %s: %s\r\n",
				ifname, brname, strerror(err));
		}
		return 1;
	}
...
static int br_cmd_setageing(int argc, char *const* argv)
{
	...
	if (err){
		fprintf(stderr, "set ageing time failed: %s\n",
			strerror(err));
		sprintf(brctl_meg_buf,"set ageing time failed: %s\r\n",
			strerror(err));
	}
	return err != 0;
}
...
static int br_cmd_setbridgeprio(int argc, char *const* argv)
{
	...
	if (err){
		fprintf(stderr, "set bridge priority failed: %s\n",
			strerror(err));
		sprintf(brctl_meg_buf,"set bridge priority failed: %s\r\n",
			strerror(err));
	}
	return err != 0;
}
...
static int br_cmd_stp(int argc, char *const* argv)
{
	...
	if (!strcmp(argv[2], "on") || !strcmp(argv[2], "yes") 
		...
		sprintf(brctl_meg_buf,"expect on/off for argument\r\n");
		return 1;
	}
	...
	if (err){
		fprintf(stderr, "set stp status failed: %s\n", 
			strerror(errno));
		sprintf(brctl_meg_buf,"%s set stp status failed: %s\r\n", 
			brctl_meg_buf,strerror(errno));
	}
	return err != 0;
}
...
static int br_cmd_showstp(int argc, char *const* argv)
{
	...
	if (br_get_bridge_info(argv[1], &info)) {
		fprintf(stderr, "%s: can't get info %s\n", argv[1],
			strerror(errno));
		sprintf(brctl_meg_buf,"%s: can't get info %s\r\n", argv[1],
			strerror(errno));
		return 1;
	}
